...
export const openDb = async (fullPath: string, createIfMissing: boolean): Promise<{ db: sqlite3.Database }> => {
...
export const initSchema = async (db: sqlite3.Database): Promise<void> => {
  await runAsync(db, 'PRAGMA foreign_keys = ON;');
  await runAsync(
    db,
    `
...
export const initInitialData = async (db: sqlite3.Database): Promise<void> => {
